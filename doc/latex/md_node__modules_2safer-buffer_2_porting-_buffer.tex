...
\hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer}{}\label{md_node__modules_2safer-buffer_2_porting-_buffer}\index{Porting to the Buffer.from/Buffer.alloc API@{Porting to the Buffer.from/Buffer.alloc API}}
\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1834}%
\Hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1834}%


\label{_overview}%
 \hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1835}{}\doxysection{\texorpdfstring{Overview}{Overview}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1835}

\begin{DoxyItemize}
\item Variant 1\+: Drop support for Node.js ≤ 4.4.x and 5.0.0 — 5.9.x. ({\itshape recommended})
\item Variant 2\+: Use a polyfill
\item Variant 3\+: manual detection, with safeguards
\end{DoxyItemize}\hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1836}{}\doxysubsection{\texorpdfstring{Finding problematic bits of code using grep}{Finding problematic bits of code using grep}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1836}
...
It will find all the potentially unsafe places in your own code (with some considerably unlikely exceptions).\hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1837}{}\doxysubsection{\texorpdfstring{Finding problematic bits of code using Node.\+js 8}{Finding problematic bits of code using Node.\+js 8}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1837}
...
\end{DoxyCode}
\hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1838}{}\doxysubsection{\texorpdfstring{Finding problematic bits of code using linters}{Finding problematic bits of code using linters}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1838}
...
\label{_variant-1}%
 \hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1839}{}\doxysection{\texorpdfstring{Variant 1\+: Drop support for Node.\+js ≤ 4.\+4.\+x and 5.\+0.\+0 — 5.\+9.\+x.}{Variant 1\+: Drop support for Node.\+js ≤ 4.\+4.\+x and 5.\+0.\+0 — 5.\+9.\+x.}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1839}
...
\label{_variant-2}%
 \hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1840}{}\doxysection{\texorpdfstring{Variant 2\+: Use a polyfill}{Variant 2\+: Use a polyfill}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1840}
Utilize \href{https://www.npmjs.com/package/safer-buffer}{\texttt{ safer-\/buffer}} as a polyfill to support older Node.\+js versions.

You would take exacly the same steps as in Variant 1, but with a polyfill `const Buffer = require(\textquotesingle{}safer-\/buffer').Buffer{\ttfamily in all files where you use the new}Buffer\`{} api.
...
\label{_variant-3}%
 \hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1841}{}\doxysection{\texorpdfstring{Variant 3 — manual detection, with safeguards}{Variant 3 — manual detection, with safeguards}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1841}
This is useful if you create Buffer instances in only a few places (e.\+g. one), or you have your own wrapper around them.\hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1842}{}\doxysubsection{\texorpdfstring{Buffer(0)}{Buffer(0)}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1842}
This special case for creating empty buffers can be safely replaced with {\ttfamily Buffer.\+concat(\mbox{[}$\,$\mbox{]})}, which returns the same result all the way down to Node.\+js 0.\+8.\+x.\hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1843}{}\doxysubsection{\texorpdfstring{Buffer(not\+Number)}{Buffer(not\+Number)}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1843}
Before\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{var\ buf\ =\ \textcolor{keyword}{new}\ Buffer(notNumber,\ \mbox{\hyperlink{negotiator_2index_8js_a9ad8213a7626ee07aef47950752cac18}{encoding}});}
...
\DoxyCodeLine{\textcolor{keywordflow}{if}\ (Buffer.from\ \&\&\ Buffer.from\ !==\ Uint8Array.from)\ \{}
\DoxyCodeLine{\ \ buf\ =\ Buffer.from(notNumber,\ \mbox{\hyperlink{negotiator_2index_8js_a9ad8213a7626ee07aef47950752cac18}{encoding}});}
\DoxyCodeLine{\}\ \textcolor{keywordflow}{else}\ \{}
\DoxyCodeLine{\ \ \textcolor{keywordflow}{if}\ (typeof\ notNumber\ ===\ \textcolor{stringliteral}{'number'})}
\DoxyCodeLine{\ \ \ \ \textcolor{keywordflow}{throw}\ \textcolor{keyword}{new}\ \mbox{\hyperlink{class_error}{Error}}(\textcolor{stringliteral}{'The\ "{}size"{}\ argument\ must\ be\ of\ type\ number.'});}
\DoxyCodeLine{\ \ buf\ =\ \textcolor{keyword}{new}\ Buffer(notNumber,\ \mbox{\hyperlink{negotiator_2index_8js_a9ad8213a7626ee07aef47950752cac18}{encoding}});}
\DoxyCodeLine{\}}
...
Also note that using Type\+Script does not fix this problem for you — when libs written in {\ttfamily Type\+Script} are used from JS, or when user input ends up there — it behaves exactly as pure JS, as all type checks are translation-\/time only and are not present in the actual JS code which TS compiles to.\hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1844}{}\doxysubsection{\texorpdfstring{Buffer(number)}{Buffer(number)}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1844}
For Node.\+js 0.\+10.\+x (and below) support\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{var\ buf;}
\DoxyCodeLine{\textcolor{keywordflow}{if}\ (Buffer.alloc)\ \{}
\DoxyCodeLine{\ \ buf\ =\ Buffer.alloc(number);}
\DoxyCodeLine{\}\ \textcolor{keywordflow}{else}\ \{}
\DoxyCodeLine{\ \ buf\ =\ \textcolor{keyword}{new}\ Buffer(number);}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keyword}{const}\ buf\ =\ Buffer.alloc\ ?\ Buffer.alloc(number)\ :\ \textcolor{keyword}{new}\ Buffer(number).fill(0);}

\end{DoxyCode}
\hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1845}{}\doxysection{\texorpdfstring{Regarding Buffer.\+alloc\+Unsafe}{Regarding Buffer.\+alloc\+Unsafe}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1845}
...
\label{_faq}%
 \hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1846}{}\doxysection{\texorpdfstring{FAQ}{FAQ}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1846}
\label{_design-flaws}%
 \hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1847}{}\doxysubsection{\texorpdfstring{What is wrong with the {\ttfamily Buffer} constructor?}{What is wrong with the {\ttfamily Buffer} constructor?}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1847}
...
\label{_ecosystem-usage}%
 \hypertarget{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1848}{}\doxysubsection{\texorpdfstring{The {\ttfamily Buffer()} constructor has been deprecated for a while. Is this really an issue?}{The {\ttfamily Buffer()} constructor has been deprecated for a while. Is this really an issue?}}\label{md_node__modules_2safer-buffer_2_porting-_buffer_autotoc_md1848}
